...
export const StandalonePrintView = () => {
      ...
      fetch(`/api/players/${id}`)
        ...
        .then(data => {
          ...
          const mapped = {
             ...
             trainingCenterAddress: data.training_center_address,
             aadharFile: data.aadhar_url,
             photoFile: data.photo_url,
             signatureFile: data.signature_url,
             guardianSignatureFile: data.guardian_signature_url,
             date: data.date
          };
          setReg(mapped);
  ...
  if (!reg) return <div className="p-10 text-center font-bold">Loading...</div>;

  return <PrintableView reg={reg} onClose={() => window.close()} isStandalone={true} />;
};

export const PrintableView = ({ reg, onClose, isStandalone }: { reg: any; onClose: () => void, isStandalone?: boolean }) => {
  if (!reg) return null;
            ...
            <div className="relative text-center mb-6 border-b-[2px] border-black pb-2">
              ...
              <div className="flex justify-between items-center w-full mb-1 mt-2">
                 ...
                 <div className="flex-1 flex flex-col items-center justify-center px-1">
                    ...
                    <div className="text-[15.5px] font-black uppercase tracking-tight leading-[1.2] text-[#000] text-center w-full whitespace-nowrap">Uttar Pradesh Taekwondo Association (UPTA)</div>
                    <div className="text-[15.5px] font-black uppercase tracking-tight leading-[1.2] text-[#000] text-center w-full whitespace-nowrap">Taekwondo Federation of India (TFI)</div>
           ...
           <div className="flex-1 mt-4">
             ...
             <div className="grid grid-cols-2 border border-black border-t-0 text-[14px] font-bold text-black h-[140px] bg-transparent">
               <div className="border-r border-black p-4 flex flex-col justify-between">
                 <div className="text-left text-[13px] font-bold">Parent / Guardian Signature</div>
                 {reg.guardianSignatureFile && (
                   <div style={{height:'60px', marginTop:'8px', overflow:'hidden'}}>
                     <img src={reg.guardianSignatureFile} style={{width:'100%', height:'100%', objectFit:'fill', display:'block'}}/>
                   </div>
                 )}
               </div>
