...
export const AdminLayout = () => {
  ...
  const menuItems = [
    { path: '/dashboard-admin', label: 'Players', icon: <Users className="w-5 h-5" /> },
    { path: '/dashboard-admin/categories', label: 'Weight Categories', icon: <Settings className="w-5 h-5" /> },
    { path: '/dashboard-admin/tiesheets', label: 'Draws & Brackets', icon: <Target className="w-5 h-5" /> },
    { path: '/dashboard-admin/reports', label: 'Export Reports', icon: <Download className="w-5 h-5" /> },
    { path: '/dashboard-admin/settings', label: 'Security', icon: <Shield className="w-5 h-5" /> },
  ];
      ...
      <aside className="w-64 bg-[#0a0a0a] border-r border-white/5 text-white flex flex-col justify-between print:hidden">
        <div>
          <div className="p-8">
            <h1 className="flex items-center gap-3 drop-shadow-[0_2px_10px_rgba(255,0,0,0.2)]">
              <button onClick={handleReturnToWebsite} className="bg-white p-1 rounded-xl border border-red-500/50 shadow-[0_0_15px_rgba(255,0,0,0.3)] shrink-0 group/logo overflow-hidden h-11 w-11 flex items-center justify-center cursor-pointer">
        ...
        <div className="p-6 border-t border-white/5 bg-[#111] space-y-3">
          <button 
            ...
            className="flex w-full items-center justify-center gap-3 px-5 py-3.5 text-gray-400 hover:bg-white/5 hover:border-white/10 hover:text-white border border-white/5 rounded-xl transition-all font-bold text-[12px] uppercase tracking-widest shadow-inner group cursor-pointer"
          >
