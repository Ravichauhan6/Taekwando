...
import { Globe, Users, Newspaper, Bell, LogOut, Shield, Award, Building, Building2, Image as ImageIcon, FileText, Settings, Trophy, MapPin, Quote, Video, Calendar, Settings2 } from 'lucide-react';
...
export const AdminLayout = () => {
  ...
  const menuItems = [
    { path: '/admin-dashboard', label: 'Registrations Data', icon: <Users className="w-5 h-5" /> },
    { path: '/admin-dashboard/promo-videos', label: 'Promo Videos', icon: <Video className="w-5 h-5" /> },
    { path: '/admin-dashboard/belts', label: 'Belt Management', icon: <Award className="w-5 h-5" /> },
    { path: '/admin-dashboard/academy', label: 'Coach/Academy', icon: <Building className="w-5 h-5" /> },
    { path: '/admin-dashboard/training-centers', label: 'Training Centers', icon: <MapPin className="w-5 h-5" /> },
    { path: '/admin-dashboard/camps', label: 'Upcoming Camps', icon: <Calendar className="w-5 h-5" /> },
    { path: '/admin-dashboard/events', label: 'Event Creation', icon: <Trophy className="w-5 h-5" /> },
    { path: '/admin-dashboard/gallery', label: 'Media Gallery', icon: <ImageIcon className="w-5 h-5" /> },
    { path: '/admin-dashboard/documents', label: 'Documents', icon: <FileText className="w-5 h-5" /> },
    { path: '/admin-dashboard/news', label: 'News Updates', icon: <Newspaper className="w-5 h-5" /> },
    { path: '/admin-dashboard/notifications', label: 'Notifications', icon: <Bell className="w-5 h-5" /> },
    { path: '/admin-dashboard/what-we-say', label: 'Leadership Messages', icon: <Quote className="w-5 h-5" /> },
    { path: '/admin-dashboard/referees', label: 'National Referees', icon: <Shield className="w-5 h-5" /> },
    ...
    <div className="flex h-screen bg-[#050505] font-sans text-gray-200 selection:bg-red-500/30">
      ...
      <aside className="w-72 bg-[#111] border-r border-white/5 flex flex-col shadow-[10px_0_30px_rgba(0,0,0,0.5)] z-20 relative">
        <div className="p-6 pb-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
               ...
               <h2 className="text-[10px] text-red-400 font-bold uppercase tracking-widest mt-0.5">Admin</h2>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-8 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => {
            const isActive = location.pathname === item.path || (item.path !== '/admin-dashboard' && item.path !== '/' && location.pathname.startsWith(item.path));
            return (
              <Link
                key={item.path}
                to={item.path}
